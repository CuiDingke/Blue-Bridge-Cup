...
void TIM3_IRQHandler(void)
{
  if (TIM_GetITStatus(TIM3, TIM_IT_CC1) != RESET)
  {
    ...
    capture1 = TIM_GetCapture1(TIM3);
		...
		{
			i++;
	    TIM_SetCompare1(TIM3, capture1 + 36000-CCR1_Val );	
		}
		else
		{
			i=0;
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_CC2) != RESET)
  {
    ...
    capture2 = TIM_GetCapture2(TIM3);
		...
		{
			j++;
	    TIM_SetCompare2(TIM3, capture2 + 18000-CCR2_Val );	
		}
		else
		{
			j=0;
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_CC3) != RESET)
  {
    ...
    capture3 = TIM_GetCapture3(TIM3);
		...
		{
			m++;
	    TIM_SetCompare3(TIM3, capture3 + 9000-CCR3_Val );	
		}
		else
		{
			m=0;
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_CC4) != RESET)
  {
    ...
    capture4 = TIM_GetCapture4(TIM3);
		...
		{
			n++;
	    TIM_SetCompare4(TIM3, capture4 + 4500-CCR4_Val );	
		}
		else
		{
			n=0;
